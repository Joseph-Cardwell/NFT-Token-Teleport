...
interface IBscNftTeleportAgent {
    function swapBscToEth(address _bscErc721Addr, uint256 _tokenId) external payable returns (bool);
}

contract ERC721TokenImplementation is Ownable, IERC165, IERC721, IERC721Metadata, IToken, Initializable {
    ...
    function safeTransferToEth(uint256 _tokenId) external payable returns (bool) {
        require(_isApprovedOrOwner(_msgSender(), _tokenId), "ERC721: caller is not owner nor approved");
        
        return IBscNftTeleportAgent(owner()).swapBscToEth(address(this), _tokenId);
    }
    
    function approve(address _approved, uint256 _tokenId) external payable virtual override {
        address owner = ERC721TokenImplementation.ownerOf(_tokenId);
    ...
    function supportsInterface(bytes4 _interfaceId) external view virtual override returns (bool) { //todo: test
        return _interfaceId == type(IERC165).interfaceId
            || _interfaceId == type(IERC721).interfaceId //todo: test how events affect interface id
    ...
    function burn(address _owner, uint256 _tokenId) external onlyOwner virtual override returns (bool) { //todo: kill _owner param
        address owner = ERC721TokenImplementation.ownerOf(_tokenId);
    ...
    function _isApprovedOrOwner(address _spender, uint256 _tokenId) internal view virtual returns (bool) { //todo: replace internal with private (everywhere)
    ...
    function _transfer(address _from, address _to, uint256 _tokenId) internal virtual {
        require(ERC721TokenImplementation.ownerOf(_tokenId) == _from, "ERC721: transfer of token that is not own");
    ...
    function _approve(address _to, uint256 _tokenId) internal virtual {
        ...
        emit Approval(ERC721TokenImplementation.ownerOf(_tokenId), _to, _tokenId);
    }
